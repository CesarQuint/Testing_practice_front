...
<Search on:enter={ getUsers } bind:value={ query.find } >
    <Button on:click={() => UserStore.modalCreate()} text="Agregar" icon="plus" color="primary" />
</Search>

<Table bind:query items={ $UsersStore.length } on:change={ getUsers } { metadata } { loading }>
    <thead>
        <th>#</th>
        <th>Nombre</th>
        <th>Correo</th>
